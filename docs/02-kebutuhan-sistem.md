...
| Tabel           | Kolom kunci                                                      |
| --------------- | ---------------------------------------------------------------- |
| `detail_masuk`  | `id_masuk`, `id_barang`, `quantity`, `harga_satuan`, `subtotal`  |
| `detail_keluar` | `id_keluar`, `id_barang`, `quantity`, `harga_satuan`, `subtotal` |

- Master sparepart & aksesoris disimpan di tabel unifikasi **`barang`** (`tipe_barang`: `sparepart` | `aksesoris`)
- `id_barang`: FK ke `barang.id_barang`
...
**Update otomatis** saat transaksi **disimpan**. Saat transaksi **dihapus**, stok di-rollback (lihat § Hapus Transaksi).

### Hapus Transaksi

| Aturan                       | Nilai                                                                     |
| ---------------------------- | ------------------------------------------------------------------------- |
| Hapus transaksi masuk/keluar | **Mengembalikan stok** (rollback efek simpan)                             |
| Hapus barang masuk           | Stok dikurangi sesuai qty transaksi (gagal jika stok sudah terpakai)      |
| Hapus barang keluar          | Stok ditambah kembali sesuai qty transaksi                                |
| Dampak                       | Data transaksi terhapus; stok dikoreksi seolah transaksi tidak pernah ada |
...
| Sparepart     | `kode_barang`  | `SP-2026-0001`  | Auto-generate saat simpan (`tipe_barang=sparepart`) |
| Aksesoris     | `kode_barang`  | `AK-2026-0001`  | Auto-generate saat simpan (`tipe_barang=aksesoris`) |

Setiap barang memiliki **`kode_barang`** (auto, wajib, unik) dan **`kode_manual`** (input opsional, kode referensi toko/supplier).
...
| ---------------------- | ------- | ------------------------------------------- |
| kode_barang            | string  | Auto-generate (SP-YYYY-NNNN), unik          |
| kode_manual            | string  | Input manual opsional                       |
| nama_barang            | string  | Nama sparepart                              |
...
**Detail item:** `id_barang` (FK ke `barang`), `quantity`, `harga_satuan`, `subtotal`. Auto-update stok saat simpan; stok di-rollback saat hapus.
